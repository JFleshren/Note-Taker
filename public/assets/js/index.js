  ...
  const handleNoteDelete = (e) => {
    ...
    deleteNote(noteId).then(() => {
      getAndRenderNotes().then((notes) => {
        ...
        // Check if jsonNotes is empty
        if (jsonNotes.length === 0) {
          // Display a message indicating no saved notes
          const noNotesMessage = createLi('No saved Notes', false);
          noteListItems.push(noNotesMessage);
        } else {
          // Iterate over jsonNotes and create list items
          jsonNotes.forEach((note) => {
            ...
            noteListItems.push(li);
                ...
                noteList.forEach((el) => {
                  el.innerHTML = '';
                  noteListItems.forEach((note) => el.append(note));
              });
          });
           renderActiveNote();
      });
